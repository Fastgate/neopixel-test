...
public:
	...
	bool update(CFastLED * fastLed) {
		unsigned long deltaTime = millis() - lastUpdate;
		bool dirty = false;
		if (deltaTime >= this->updateTime) {
			dirty = this->beforeFrame(this->ledCount, deltaTime);
			if (dirty) {
				int pixelIndex = 0;
	...
	virtual bool beforeFrame(int pixelCount, unsigned long deltaTime) = 0;
	virtual void onFrame(struct CRGB * pixel, int pixelIndex, int pixelCount,
			unsigned long deltaTime) = 0;
	virtual void afterFrame(int pixelCount, unsigned long deltaTime) = 0;
};
